...
router.get('/', async (req, res) => {
  try {
    ...
    const sqlString = 'SELECT * FROM CTNK';
    ...
    res.render('CTNK/index', {
      title: "Sensor List",
      CTNK: result.recordset,
    });
  } catch (err) {
    // Xử lý lỗi nếu xảy ra
    console.error("Lỗi truy vấn: ", err);
    res.status(500).send("Đã xảy ra lỗi khi lấy danh sách thông tin kho.");
  }
});
